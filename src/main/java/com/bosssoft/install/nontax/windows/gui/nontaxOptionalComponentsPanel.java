...
import com.bosssoft.platform.installer.wizard.gui.OptionalComponentsPanel;
import com.bosssoft.platform.installer.wizard.gui.OptionalComponentsPanel.ChooseActionListener;
import com.bosssoft.platform.installer.wizard.gui.OptionalComponentsPanel.TreeMouseListener;
...
public class nontaxOptionalComponentsPanel extends AbstractSetupPanel implements TreeSelectionListener{
	...
	public nontaxOptionalComponentsPanel() {
		try {
			jbInit();
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	private void initTree() {
		...
		//未授权应用不可勾选
        for (Object object : optionsList) {
        	ModuleDef model=(ModuleDef) object;
        	//if(!licenseApps.contains(model.getNameKey())) model.setSelected("false");
           License license=LicenseManager.getLicense(model.getNameKey());
           if(license==null) {
	...
	public class ChooseActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent ae) {
			nontaxOptionalComponentsPanel.this.calculateTotalSize();
		}
	}
